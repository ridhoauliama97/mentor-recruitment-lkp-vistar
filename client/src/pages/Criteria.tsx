    ...
    <div className="bg-muted/20 px-6 py-4">
      {items.length > 0 && (
        <div className="space-y-1 mb-3">
          {items.map((sc) => {
            const c = weightColors[sc.weight] ?? "bg-gray-100";
            return (
              <div key={sc.id} className="flex items-center justify-between py-1.5 px-3 rounded bg-white border">
                ...
                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => removeSubCriteria(sc.id, criteriaId)}>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setShowForm(true); setEditing(null); setForm({ name: "", description: "", type: "benefit", code: nextCode, weightRef: 0, status: "active" }); }}>
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle>{editing ? "Edit Kriteria" : "Tambah Kriteria Baru"}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  placeholder="Contoh: Kompetensi Teknis AI Engineer"
                />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Deskripsi / Alasan Bobot</label>
                <input
                  className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
